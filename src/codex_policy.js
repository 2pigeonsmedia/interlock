...
function extractPrefixPatterns(text) {
  const blocks = [...String(text).matchAll(/prefix_rule\(\s*pattern\s*=\s*\[([\s\S]*?)\]/g)];
  return blocks.map(block => block[1].split(',').map(part => {
...
function structurallyClosed(text, parsed) {
  const patterns = extractPrefixPatterns(text);
  if (patterns.length < 1 || patterns.some(pattern => pattern.includes(undefined))) return false;
  if (/prefix_rule/.test(text.replace(/prefix_rule\(\s*pattern\s*=\s*\[[\s\S]*?\][\s\S]*?\)/g, ''))) {
    return false;
  }
  const aliases = new Set([
    ...invocationAliases(parsed.nodePath),
    ...invocationAliases(parsed.scriptPath),
  ]);
  return patterns.every(pattern => {
    if (!pattern.length || !aliases.has(pattern[0]) || !aliases.has(pattern[1])) return false;
    if (pattern[2] === 'history') {
      return pattern.length === 7 && pattern[3] === '--connection' &&
        pattern[4] === parsed.connection && pattern[5] === '--drain' && pattern[6] === '--json';
    }
    if (pattern[2] === 'say') {
      return parsed.mode === 'participate' && pattern.length === 6 &&
        pattern[3] === '--connection' && pattern[4] === parsed.connection && pattern[5] === '--file';
    }
    return false;
  });
}
...
function parseOwned(text) {
  ...
  if (version === 3) {
    try {
      if (generatePolicy(parsed) !== text) return null;
    } catch (_) { return null; }
  } else if (!structurallyClosed(text, parsed)) {
    return null;
  }
  return parsed;
}
    ...
    try {
      ...
      if (predecessor !== null) {
        ...
        try {
          ...
          if (!ioFs.existsSync(policyPaths(rulesDir).owned)) {
            try { ioFs.renameSync(claimPath, policyPaths(rulesDir).owned); } catch (_) { /* keep claim */ }
          }
          throw error;
        }
      } else {
