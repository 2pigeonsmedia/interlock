...
function sealedPolicy(marker, spec, rules) {
  const body = [
    `# ${marker}`,
    '# Generated by interlock codex-policy. Do not edit.',
    spec.ownedLine || null,
    ...
    `# mode=${spec.mode}`,
    spec.host ? `# host=${spec.host}` : null,
    `# node=${spec.nodePath}`,
    `# script=${spec.scriptPath}`,
    spec.requestId !== undefined ? `# request_id=${spec.requestId || ''}` : null,
    spec.subjectId !== undefined ? `# subject_id=${spec.subjectId || ''}` : null,
    '',
    rules,
...
function generatePolicyV1(options) {
  ...
  const history = prefixRule(
    historyPattern(spec.nodePath, spec.scriptPath, spec.connection),
    'Interlock receive: canonical history --drain --json for this named connection. Drain acknowledges addressed rows and advances that connection cursor.',
  );
  const say = spec.mode === 'participate' ? prefixRule(
    sayPattern(spec.nodePath, spec.scriptPath, spec.connection),
    'Interlock participate: canonical say --file for this named connection. Codex may send any readable file through this seat without per-message Auto-review. No payload inspection, classification, redaction, or restriction.',
  ) : '';
  return sealedPolicy(MARKER_V1, spec, history + say);
}
...
function generatePolicyV2(options) {
  const spec = {
    ...
    nodePath: options.nodePath,
    scriptPath: options.scriptPath,
    ownedLine: '# One Interlock-owned file. Installing for a connection replaces any previous Interlock policy.',
  };
  const pairs = [];
  for (const node of invocationAliases(spec.nodePath)) {
    for (const script of invocationAliases(spec.scriptPath)) {
      pairs.push([node, script]);
    }
  }
  const history = pairs.map(([node, script]) => prefixRule(
    historyPattern(node, script, spec.connection),
    'Interlock receive: canonical history --drain --json for this named connection. Drain acknowledges addressed rows and advances that connection cursor.',
  )).join('');
  const say = spec.mode === 'participate' ? pairs.map(([node, script]) => prefixRule(
    sayPattern(node, script, spec.connection),
    'Interlock participate: canonical say --file for this named connection. Codex may send any readable file through this seat without per-message Auto-review. No payload inspection, classification, redaction, or restriction.',
  )).join('') : '';
  return sealedPolicy(MARKER_V2, spec, history + say);
}
