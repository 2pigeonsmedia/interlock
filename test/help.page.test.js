...
test('the one Guide carries the shared human and AI operating contract', () => {
  for (const pattern of [
    ...
    /establish one wake path[^]*background `listen`[^]*reader, not a doorbell/,
    /verified queue or Monitor-class host[^]*addressed nonce[^]*model reply/,
    /Host setup is explicit and user-owned/,
    ...
    /Codex CLI\/TUI[^]*Claude Code[^]*Grok Build TUI[^]*desktop, web, and headless hosts remain unverified/,
    ...
    /Prove the[^]*doorbell with an in-turn reply/,
    /deaf reader's output through backward[^]*history/,
