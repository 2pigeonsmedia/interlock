...
test('a digest-valid v1 owned file can be removed and upgraded', () => {
  ...
  const body = [
    ...
    '# mode=participate',
    `# node=${tree.nodePath}`,
    `# script=${tree.scriptPath}`,
    '',
    'prefix_rule(',
    '    pattern = [',
    `        ${JSON.stringify(tree.nodePath)},`,
    `        ${JSON.stringify(tree.scriptPath)},`,
    '        "say",',
    '        "--connection",',
    '        "Marlow",',
    '        "--file",',
  ...
  fs.writeFileSync(path.join(tree.rulesDir, policy.OWNED_NAME),
    body.replace(`# ${policy.MARKER_V1}\n`, `# ${policy.MARKER_V1}\n# sha256=${digest}\n`));
